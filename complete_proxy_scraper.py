# Automated JDownloader2 and MegaBasterd Proxy Lists
# Copyright (C) 2026 Robert Riordan
#
...
def get_proxy_priority(proxy: ProxyRecord) -> int:
    """Return priority value for deduplication - lower number = higher priority"""
...
class ProxyScraper:
        ...
        try:
            ...
            for i in range(0, len(tasks), ProxyScraperConfig.BATCH_SIZE):
                ...
                if current_time - self.last_progress_update >= self.progress_update_interval:
                    ...
                    logger.info(f"SOCKS5-PRIORITY: {progress:.1f}% | "
    ...
    def save_all_proxy_files(self, proxies: List[ProxyRecord]):
        ...
        logger.info("\nFinal Proxy Distribution (SOCKS5-Prioritized):")
    ...
    def generate_comprehensive_report(self):
        ...
        report_data = {
            "scraping_session": {
                ...
                "deduplication_mode": "SOCKS5_PRIORITIZED",
        ...
        summary_lines = [
            "="*80,
            f"SOCKS5-PRIORITIZED PROXY SCRAPING REPORT{mode_str}",
            "="*80,
            ...
            f"Deduplication: SOCKS5-PRIORITIZED (SOCKS5 > SOCKS4 > HTTP)",
            "",
            ...
            "✓ SOCKS5-Prioritized Deduplication (better proxy quality)",
            "",
            "TIMING BREAKDOWN",
            "-" * 50,
            f"Source Fetching: {self.format_duration(self.stats.timing.fetching_duration)}",
            f"SOCKS5-Prioritized Deduplication: {self.format_duration(self.stats.timing.deduplication_duration)}",
        ...
        summary_lines.extend([
            "",
            "SOCKS5-PRIORITIZED OUTPUT STATISTICS",
            "-" * 40,
        ])
        ...
        # Console summary
        logger.info(f"\n" + "="*80)
        logger.info(f"SOCKS5-PRIORITIZED SCRAPING COMPLETED{mode_str}")
        logger.info("="*80)
        ...
        logger.info("SOCKS5-PRIORITIZED: Enhanced proxy quality with SOCKS5 preference!")
        logger.info("="*80)

def parse_arguments():
    parser = argparse.ArgumentParser(description='SOCKS5-Prioritized Multi-Source Proxy Generator')
...
async def main():
    ...
    logger.info("="*80)
    logger.info("SOCKS5-PRIORITIZED MULTI-SOURCE PROXY SCRAPER")
    logger.info("="*80)
    ...
    logger.info("Optimizations: uvloop, TCPConnector, Parallel Fetching, Pre-filtering")
    logger.info("Deduplication: SOCKS5-PRIORITIZED (SOCKS5 > SOCKS4 > HTTP)")
    logger.info("="*80)
    ...
    # STAGE 2: SOCKS5-PRIORITIZED CROSS-SOURCE DEDUPLICATION
    logger.info("\nSTAGE 2: SOCKS5-PRIORITIZED CROSS-SOURCE DEDUPLICATION")
    scraper.stats.timing.deduplication_start_time = time.time()

    # Sort proxies by priority: SOCKS5 > SOCKS4 > HTTP
    all_proxies_sorted = sorted(all_proxies, key=get_proxy_priority)

    seen_global_proxies = set()
    unique_proxies = []
    priority_stats = defaultdict(int)  # Track what got kept

    for proxy in all_proxies_sorted:
        proxy_key = f"{proxy.proxy.address}:{proxy.proxy.port}"
        if proxy_key not in seen_global_proxies:
            seen_global_proxies.add(proxy_key)
            unique_proxies.append(proxy)
            priority_stats[proxy.proxy.type] += 1
    ...
    logger.info(f"SOCKS5-Prioritized results:")
    for proxy_type, count in sorted(priority_stats.items()):
        logger.info(f"   • {proxy_type}: {count:,} unique IP:port combinations kept")
    ...
    logger.info("\nSTAGE 3: SOCKS5-PRIORITIZED PROXY VALIDATION")
    ...
    # Final summary
    logger.info(f"\nSOCKS5-PRIORITIZED MULTI-SOURCE SCRAPING COMPLETED!")
